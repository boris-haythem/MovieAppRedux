      ...
      {movieslist
        ...
        .map(el => (
          <div className="moviecontainer">
            ...
            <p className="moviename">{el.movie.name}</p><div style={{display:'flex',width:'100%',justifyContent:'space-around'}}>
            <Edit id={el.id}/><button onClick={()=>remove_movie(el.id)}>Delete</button></div >
          </div>
        ))}
      <div className={"modalcontainer moviecontainer"}><Modal/></div>
    </div>
  );}
...
const MapStateToProps = state => {return{movieslist:state.movieslist ,searchvalue:state.searchvalue}};
